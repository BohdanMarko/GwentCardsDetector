...
const string InputImagePath = @".\input\20241102_155744.jpg";
const string TemplatesPath = @".\templates";
const string ResultImagePath = @".\result.jpg";

Process(InputImagePath);

void Process(string inputImagePath)
{
...
string ResolveDeckName(Image<Rgba32> inputCard)
{
    ...
    inputCard.Mutate(x => x.AutoOrient().Saturate(2f));

    foreach (string template in templates)
    {
        using Image<Rgba32> templateImage = Image.Load<Rgba32>(template);
        templateImage.Mutate(x => x.Saturate(2f));
        inputCard.Mutate(ctx => ctx.Resize(templateImage.Size));
        templateImage.Save(@$"E:\projects\GwentCardsDetector\template.jpg");
        inputCard.Save(@$"E:\projects\GwentCardsDetector\input.jpg");
